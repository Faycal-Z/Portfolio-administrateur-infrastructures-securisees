...
Rendez-vous sur https://mail.oclock.school/ pour consulter vos mails O'clock.
...
# Étape 3 - pfSense
💡 pfSense est un système d'exploitation permettant de transformer n'importe quel ordinateur en un routeur professionnel. Il n'est pas rare d'en rencontrer en entreprise ! Si vous voulez en savoir plus, jetez un oeil par ici.
...
💡 Petit indice : on ne veut pas récupérer une adresse via DHCP, mais attribuer une addresse IP fixe sur le pont vmbr1 (notre switch virtuel entre la pfSense et Proxmox). Regardez le diagramme de l'étape 2 pour déterminer l'adresse à utiliser sur la patte WAN de la pfSense !
...
➡️ Depuis un navigateur web sur la VM Windows 10, rendez-vous sur l'interface de la pfSense à l'adresse 192.168.1.1.
...
Récupérez votre fichier (en fonction de votre utilisateur) sur votre PC en vous l'envoyant par WeTransfer (par exemple) depuis la VM Windows 10.
...
Installez sur votre ordinateur (pas sur une VM !) le logiciel OpenVPN Connect.
...
Ouvrez votre navigateur web (sur votre machine, pas dans une VM !) et essayez d'accéder à https://10.0.0.1/. Vous devriez voir la page de connexion de votre pfSense, ça veut dire que le VPN fonctionne 🎉
...
- Création de pfSense (incluant vmbr1, vmbr2 et vmbr3)
